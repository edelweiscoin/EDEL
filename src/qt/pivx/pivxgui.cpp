...
void PIVXGUI::createTrayIcon(const NetworkStyle* networkStyle)
{
    ...
    QString toolTip = tr("Edelweis client") + " " + networkStyle->getTitleAddText();
...
{
    QString strTitle =  tr("Edelweis"); // default title
...
bool PIVXGUI::openStandardDialog(QString title, QString body, QString okBtn, QString cancelBtn)
{
    ...
    if (isVisible()) {
        ...
        dialog->setWindowTitle(tr("Edelweis"));
        dialog->adjustSize();
        dialog->raise();
        dialog->exec();
    }
