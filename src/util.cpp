...
const char * const PIVX_CONF_FILENAME = "edelweis.conf";
const char * const PIVX_PID_FILENAME = "edelweis.pid";
const char * const PIVX_MASTERNODE_CONF_FILENAME = "masternode.conf";


// Edelweis only features
// Masternode
...
{
// Windows < Vista: C:\Documents and Settings\Username\Application Data\edelweis
// Windows >= Vista: C:\Users\Username\AppData\Roaming\edelweis
// Mac: ~/Library/Application Support/edelweis
// Unix: ~/.edelweis
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Edelweis";
#else
    fs::path pathRet;
...
static fs::path ZC_GetBaseParamsDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\edelweisParams
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\edelweisParams
    // Mac: ~/Library/Application Support/edelweisParams
    // Unix: ~/.edelweis-params
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "EdelweisParams";
#else
    fs::path pathRet;
    char* pszHome = getenv("HOME");
    if (pszHome == NULL || strlen(pszHome) == 0)
    ...
    pathRet /= "Library/Application Support";
    TryCreateDirectory(pathRet);
    return pathRet / "EdelweisParams";
#else
    // Unix
    return pathRet / ".edelweis-params";
#endif
#endif
}
    ...
    std::map<std::string, std::vector<std::string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()) {
        // Create empty edelweis.conf if it does not exist
        FILE* configFile = fsbridge::fopen(GetConfigFile(), "a");
        if (configFile != NULL){
            std::string strHeader = "# Edelweis config file\n"
                          "rpcuser=username\n"
    ...
    for (boost::program_options::detail::config_file_iterator it(streamConfig, setOptions), end; it != end; ++it) {
        // Don't overwrite existing settings so command line settings override edelweis.conf
